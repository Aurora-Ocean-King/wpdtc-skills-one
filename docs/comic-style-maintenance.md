# Style Maintenance (wpdtc-comic)

## Adding a New Style

1. Create style definition: `skills/wpdtc-comic/references/styles/<style-name>.md`
2. Update SKILL.md: add to `--style` options table + auto-selection entry
3. Generate showcase image:
   ```bash
   ${BUN_X} skills/wpdtc-danger-gemini-web/scripts/main.ts \
   ...
   ```
4. Compress: `${BUN_X} skills/wpdtc-compress-image/scripts/main.ts screenshots/comic-styles/<style-name>.png`
