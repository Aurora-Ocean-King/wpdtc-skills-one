...
metadata:
  openclaw:
    homepage: https://github.com/Aurora-Ocean-King/wpdtc-skills-one#<skill-name>
...
`packages/` is the source of truth for shared runtime code. Most skills consume shared packages from npm with semver ranges. `wpdtc-url-to-markdown` is the exception: it vendors the `wpdtc-fetch` runtime into `skills/wpdtc-url-to-markdown/scripts/lib/` so the published skill is self-contained and does not depend on the `wpdtc-fetch` npm package.

Current packages:
- `wpdtc-chrome-cdp` (Chrome CDP utilities), consumed by 5 skills (`wpdtc-danger-gemini-web`, `wpdtc-danger-x-to-markdown`, `wpdtc-post-to-wechat`, `wpdtc-post-to-weibo`, `wpdtc-post-to-x`)
- `wpdtc-md` (shared Markdown rendering and placeholder pipeline), consumed by 3 skills (`wpdtc-markdown-to-html`, `wpdtc-post-to-wechat`, `wpdtc-post-to-weibo`)
- `wpdtc-fetch` (URL-to-Markdown CLI), vendored into 1 skill (`wpdtc-url-to-markdown`)
...
2. Run the package build, e.g. `bun run --cwd packages/wpdtc-md build`
