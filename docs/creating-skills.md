...
| Requirement | Details |
|-------------|---------|
| **Prefix** | All skills MUST use `wpdtc-` prefix |
...
metadata:
  openclaw:
    homepage: https://github.com/Aurora-Ocean-King/wpdtc-skills-one#wpdtc-<name>
...
1. Create `skills/wpdtc-<name>/SKILL.md` with YAML front matter
2. Add TypeScript in `skills/wpdtc-<name>/scripts/` (if applicable)
3. Add prompt templates in `skills/wpdtc-<name>/prompts/` if needed
4. Register the skill in `.claude-plugin/marketplace.json` under the `wpdtc-skills` plugin entry
5. Add Script Directory section to SKILL.md if skill has scripts
6. Add openclaw metadata to frontmatter

## Skill Grouping

All skills are registered under the single `wpdtc-skills` plugin. Use these logical groups when deciding where the skill should appear in the docs:
...
If you add a new logical group, update the docs that present grouped skills, but keep the skill registered under the single `wpdtc-skills` plugin entry.
...
\`\`\`bash
test -f .wpdtc-skills/<skill-name>/EXTEND.md && echo "project"
test -f "${XDG_CONFIG_HOME:-$HOME/.config}/wpdtc-skills/<skill-name>/EXTEND.md" && echo "xdg"
test -f "$HOME/.wpdtc-skills/<skill-name>/EXTEND.md" && echo "user"
\`\`\`

| Path | Location |
|------|----------|
| `.wpdtc-skills/<skill-name>/EXTEND.md` | Project directory |
| `$XDG_CONFIG_HOME/wpdtc-skills/<skill-name>/EXTEND.md` | XDG config (~/.config) |
| `$HOME/.wpdtc-skills/<skill-name>/EXTEND.md` | User home (legacy) |
...
- **Use whatever image-generation tool or skill is available** in the current runtime — e.g., Codex `imagegen`, Hermes `image_generate`, `wpdtc-imagine`, or any equivalent the user has installed.
...
Concrete tool names (`imagegen`, `image_generate`, `wpdtc-imagine`) above are examples — substitute the local equivalents under the same rule.
```
