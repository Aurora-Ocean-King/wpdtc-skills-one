...
test("verify-shared-package-deps accepts npm semver dependencies", async (t) => {
  const root = await makeTempDir("wpdtc-verify-shared-ok-");
  t.after(() => fs.rm(root, { recursive: true, force: true }));

  await writeJson(path.join(root, "packages", "wpdtc-md", "package.json"), {
    name: "wpdtc-md",
    version: "1.2.3",
  });
...
test("verify-shared-package-deps rejects file dependencies and vendored workspace packages", async (t) => {
  const root = await makeTempDir("wpdtc-verify-shared-bad-");
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  ...
  await writeJson(path.join(root, "skills", "demo", "scripts", "package.json"), {
    dependencies: {
      "wpdtc-md": "file:./vendor/wpdtc-md",
    },
  });
  await writeFile(path.join(root, "skills", "demo", "scripts", "vendor", "wpdtc-md", "index.ts"));
