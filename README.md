...
WPDTC skills collection for improving daily work efficiency with Claude Code.
...
```bash
npx skills add Aurora-Ocean-King/wpdtc-skills-one
```

### Publish to ClawHub / OpenClaw

This repository now supports publishing each `skills/wpdtc-*` directory as an individual ClawHub skill.
...
```bash
clawhub install wpdtc-imagine
clawhub install wpdtc-markdown-to-html
```
...
```bash
/plugin marketplace add Aurora-Ocean-King/wpdtc-skills-one
```
...
2. Select **wpdtc-skills**
3. Select the **wpdtc-skills** plugin
...
/plugin install wpdtc-skills@wpdtc-skills
```
...
> Please install Skills from github.com/Aurora-Ocean-King/wpdtc-skills-one
...
|--------|-------------|----------|
| **wpdtc-skills** | Content generation, AI backends, and utility tools for daily work efficiency | All skills in this repository, organized below as Content Skills, AI Generation Skills, and Utility Skills |
...
/wpdtc-infographic path/to/content.md

# Specify layout
/wpdtc-infographic path/to/content.md --layout pyramid

# Specify style (default: craft-handmade)
/wpdtc-infographic path/to/content.md --style technical-schematic

# Specify both
/wpdtc-infographic path/to/content.md --layout funnel --style corporate-memphis

# With aspect ratio (named preset or custom W:H)
/wpdtc-infographic path/to/content.md --aspect portrait
/wpdtc-infographic path/to/content.md --aspect 3:4
```
...
/wpdtc-cover-image path/to/article.md

# Quick mode: skip confirmation, use auto-selection
/wpdtc-cover-image path/to/article.md --quick

# Specify dimensions (5D system)
/wpdtc-cover-image path/to/article.md --type conceptual --palette cool --rendering digital
/wpdtc-cover-image path/to/article.md --text title-subtitle --mood bold

# Style presets (backward-compatible shorthand)
/wpdtc-cover-image path/to/article.md --style blueprint

# Specify aspect ratio (default: 16:9)
/wpdtc-cover-image path/to/article.md --aspect 2.35:1

# Visual only (no title text)
/wpdtc-cover-image path/to/article.md --no-title
```
...
```bash
# From markdown file
/wpdtc-slide-deck path/to/article.md

# With style and audience
/wpdtc-slide-deck path/to/article.md --style corporate
/wpdtc-slide-deck path/to/article.md --audience executives

# Target slide count
/wpdtc-slide-deck path/to/article.md --slides 15

# Outline only (no image generation)
/wpdtc-slide-deck path/to/article.md --outline-only

# With language
/wpdtc-slide-deck path/to/article.md --lang zh
```
...
/wpdtc-comic posts/turing-story/source.md

# Specify art style and tone
/wpdtc-comic posts/turing-story/source.md --art manga --tone warm
/wpdtc-comic posts/turing-story/source.md --art ink-brush --tone dramatic

# Use preset (includes special rules)
/wpdtc-comic posts/turing-story/source.md --style ohmsha
/wpdtc-comic posts/turing-story/source.md --style wuxia

# Specify layout and aspect ratio
/wpdtc-comic posts/turing-story/source.md --layout cinematic
/wpdtc-comic posts/turing-story/source.md --aspect 16:9

# Specify language
/wpdtc-comic posts/turing-story/source.md --lang zh

# Direct content input
/wpdtc-comic "The story of Alan Turing and the birth of computer science"
```
...
/wpdtc-article-illustrator path/to/article.md --style vector-illustration --palette macaron
```
...
```bash
# Post with text
/wpdtc-post-to-x "Hello from Claude Code!"

# Post with images
/wpdtc-post-to-x "Check this out" --image photo.png

# Post X Article
/wpdtc-post-to-x --article path/to/article.md
```
...
/wordpress-post "title" --markdown article.md
/wordpress-post "title" --markdown article.md --status draft
/wordpress-post "title" --markdown article.md --categories tech,ai

#### wpdtc-post-to-wechat
...
```bash
/wpdtc-post-to-wechat 贴图 --markdown article.md --images ./photos/
/wpdtc-post-to-wechat 贴图 --markdown article.md --image img1.png --image img2.png --image img3.png
/wpdtc-post-to-wechat 贴图 --title "标题" --content "内容" --image img1.png --submit
```

**Article (文章)** - Full markdown/HTML with rich formatting:

```bash
/wpdtc-post-to-wechat 文章 --markdown article.md
/wpdtc-post-to-wechat 文章 --markdown article.md --theme grace
/wpdtc-post-to-wechat 文章 --html article.html
```
...
```bash
# Add to .wpdtc-skills/.env (project-level) or ~/.wpdtc-skills/.env (user-level)
...
```bash
mkdir -p .wpdtc-skills/wpdtc-post-to-wechat
```

Create `.wpdtc-skills/wpdtc-post-to-wechat/EXTEND.md`:
...
```bash
# Post with text
/wpdtc-post-to-weibo "Hello Weibo!"

# Post with images
/wpdtc-post-to-weibo "Check this out" --image photo.png

# Post with video
/wpdtc-post-to-weibo "Watch this" --video clip.mp4
```
...
```bash
# Publish article
/wpdtc-post-to-weibo --article article.md

# With cover image
/wpdtc-post-to-weibo --article article.md --cover cover.jpg
```
...
/wpdtc-imagine --prompt "A cute cat" --image cat.png

# With aspect ratio
/wpdtc-imagine --prompt "A landscape" --image landscape.png --ar 16:9

# High quality (2k)
/wpdtc-imagine --prompt "A banner" --image banner.png --quality 2k

# Specific provider
/wpdtc-imagine --prompt "A cat" --image cat.png --provider openai --model gpt-image-2

# Azure OpenAI (model = deployment name)
/wpdtc-imagine --prompt "A cat" --image cat.png --provider azure --model gpt-image-2

# OpenRouter
/wpdtc-imagine --prompt "A cat" --image cat.png --provider openrouter

# OpenRouter with reference images
/wpdtc-imagine --prompt "Make it blue" --image out.png --provider openrouter --model google/gemini-3.1-flash-image-preview --ref source.png

# DashScope (Aliyun Tongyi Wanxiang)
/wpdtc-imagine --prompt "一只可爱的猫" --image cat.png --provider dashscope

# DashScope with custom size
/wpdtc-imagine --prompt "为咖啡品牌设计一张 21:9 横幅海报，包含清晰中文标题" --image banner.png --provider dashscope --model qwen-image-2.0-pro --size 2048x872

# Z.AI GLM-Image
/wpdtc-imagine --prompt "一张带清晰中文标题的科技海报" --image out.png --provider zai

# MiniMax
/wpdtc-imagine --prompt "A fashion editorial portrait by a bright studio window" --image out.jpg --provider minimax

# MiniMax with subject reference
/wpdtc-imagine --prompt "A girl stands by the library window, cinematic lighting" --image out.jpg --provider minimax --model image-01 --ref portrait.png --ar 16:9

# Replicate (default: google/nano-banana-2)
/wpdtc-imagine --prompt "A cat" --image cat.png --provider replicate

# Replicate Seedream 4.5
/wpdtc-imagine --prompt "A studio portrait" --image portrait.png --provider replicate --model bytedance/seedream-4.5 --ar 3:2

# Replicate Wan 2.7 Image Pro
/wpdtc-imagine --prompt "A concept frame" --image frame.png --provider replicate --model wan-video/wan-2.7-image-pro --size 2048x1152

# Jimeng (即梦)
/wpdtc-imagine --prompt "一只可爱的猫" --image cat.png --provider jimeng

# Seedream (豆包)
/wpdtc-imagine --prompt "一只可爱的猫" --image cat.png --provider seedream
...
| `WPDTC_IMAGE_GEN_MAX_WORKERS` | Override batch worker cap | `10` |
| `WPDTC_IMAGE_GEN_<PROVIDER>_CONCURRENCY` | Override provider concurrency | provider-specific |
...
- Replicate defaults to `google/nano-banana-2`. `wpdtc-imagine` only enables Replicate advanced options for `google/nano-banana*`, `bytedance/seedream-4.5`, `bytedance/seedream-5-lite`, `wan-video/wan-2.7-image`, and `wan-video/wan-2.7-image-pro`.
...
```bash
/wpdtc-danger-gemini-web "Hello, Gemini"
/wpdtc-danger-gemini-web --prompt "Explain quantum computing"
```

**Image Generation:**

```bash
/wpdtc-danger-gemini-web --prompt "A cute cat" --image cat.png
/wpdtc-danger-gemini-web --promptfiles system.md content.md --image out.png
```
...
/wpdtc-youtube-transcript https://www.youtube.com/watch?v=VIDEO_ID

# Specify languages (priority order)
/wpdtc-youtube-transcript https://youtu.be/VIDEO_ID --languages zh,en,ja

# With chapters and speaker identification
/wpdtc-youtube-transcript https://youtu.be/VIDEO_ID --chapters --speakers

# SRT subtitle format
/wpdtc-youtube-transcript https://youtu.be/VIDEO_ID --format srt

# List available transcripts
/wpdtc-youtube-transcript https://youtu.be/VIDEO_ID --list
```
...
/wpdtc-url-to-markdown https://example.com/article

# Wait mode - for login-required pages
/wpdtc-url-to-markdown https://example.com/private --wait

# Save to specific file
/wpdtc-url-to-markdown https://example.com/article -o output.md
```
...
#### wpdtc-danger-x-to-markdown

Converts X (Twitter) content to markdown format. Supports tweet threads and X Articles.

```bash
# Convert tweet to markdown
/wpdtc-danger-x-to-markdown https://x.com/username/status/123456

# Save to specific file
/wpdtc-danger-x-to-markdown https://x.com/username/status/123456 -o output.md

# JSON output
/wpdtc-danger-x-to-markdown https://x.com/username/status/123456 --json

# Download media (images/videos) to local files
/wpdtc-danger-x-to-markdown https://x.com/username/status/123456 --download-media
```
...
#### wpdtc-compress-image

Compress images to reduce file size while maintaining quality.

```bash
/wpdtc-compress-image path/to/image.png
/wpdtc-compress-image path/to/images/ --quality 80
```

#### wpdtc-format-markdown

Format plain text or markdown files with proper frontmatter, titles, summaries, headings, bold, lists, and code blocks.

```bash
# Format a markdown file
/wpdtc-format-markdown path/to/article.md

# Format with specific output
/wpdtc-format-markdown path/to/draft.md
```
...
```bash
# Basic conversion
/wpdtc-markdown-to-html article.md

# Theme + color
/wpdtc-markdown-to-html article.md --theme grace --color red

# Convert ordinary external links to bottom citations
/wpdtc-markdown-to-html article.md --cite
```
...
1. CLI environment variables (e.g., `OPENAI_API_KEY=xxx /wpdtc-imagine ...`)
2. `process.env` (system environment)
3. `<cwd>/.wpdtc-skills/.env` (project-level)
4. `~/.wpdtc-skills/.env` (user-level)

**Setup**:

```bash
# Create user-level config directory
mkdir -p ~/.wpdtc-skills

# Create .env file
cat > ~/.wpdtc-skills/.env << 'EOF'
# OpenAI
...
```bash
mkdir -p .wpdtc-skills
# Add .wpdtc-skills/.env to .gitignore to avoid committing secrets
echo ".wpdtc-skills/.env" >> .gitignore
```
...
1. `.wpdtc-skills/<skill-name>/EXTEND.md` - Project-level (for team/project-specific settings)
2. `~/.wpdtc-skills/<skill-name>/EXTEND.md` - User-level (for personal preferences)

**Example**: To customize `wpdtc-cover-image` with your brand colors:

```bash
mkdir -p .wpdtc-skills/wpdtc-cover-image
```

Then create `.wpdtc-skills/wpdtc-cover-image/EXTEND.md`:
...
```bash
HTTP_PROXY=http://127.0.0.1:7890 HTTPS_PROXY=http://127.0.0.1:7890 /wpdtc-danger-gemini-web "Hello"
```
...
[![Star History Chart](https://api.star-history.com/svg?repos=Aurora-Ocean-King/wpdtc-skills-one&type=Date)](https://www.star-history.com/#Aurora-Ocean-King/wpdtc-skills-one&Date)
