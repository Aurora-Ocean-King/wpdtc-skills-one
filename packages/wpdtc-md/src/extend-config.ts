...
export function loadExtendConfig(): Partial<ExtendConfig> {
  const paths = [
    path.join(process.cwd(), ".wpdtc-skills", "wpdtc-markdown-to-html", "EXTEND.md"),
    path.join(
      process.env.XDG_CONFIG_HOME || path.join(homedir(), ".config"),
      "wpdtc-skills", "wpdtc-markdown-to-html", "EXTEND.md"
    ),
    path.join(homedir(), ".wpdtc-skills", "wpdtc-markdown-to-html", "EXTEND.md"),
  ];
