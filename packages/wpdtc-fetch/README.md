...
`wpdtc-fetch` is a Bun CLI built on Chrome CDP. Give it a URL and it returns
...
- Chrome profile defaults to `wpdtc-skills/chrome-profile`
...
```bash
bunx wpdtc-fetch https://example.com
```

You can also install it globally:

```bash
npm install -g wpdtc-fetch
```
...
bunx wpdtc-fetch https://example.com
wpdtc-fetch https://example.com
wpdtc-fetch https://example.com --format markdown --output article.md
wpdtc-fetch https://example.com --format markdown --output article.md --download-media
wpdtc-fetch https://x.com/jack/status/20 --format json --output article.json
wpdtc-fetch https://x.com/jack/status/20 --json
wpdtc-fetch https://x.com/jack/status/20 --wait-for interaction
wpdtc-fetch https://x.com/jack/status/20 --wait-for force
wpdtc-fetch https://x.com/jack/status/20 --chrome-profile-dir ~/Library/Application\\ Support/wpdtc-skills/chrome-profile
```

## Options

```bash
wpdtc-fetch <url> [options]
...
Options:
  ...
  --chrome-profile-dir <path>
                        Chrome user data dir. Defaults to WPDTC_CHROME_PROFILE_DIR
                        or wpdtc-skills/chrome-profile
