---
name: wpdtc-url-to-markdown
description: Fetch any URL and convert to markdown using wpdtc-fetch CLI (Chrome CDP with site-specific adapters). Built-in adapters for X/Twitter, YouTube transcripts, Hacker News threads, and generic pages via Defuddle. Handles login/CAPTCHA via interaction wait modes. Use when user wants to save a webpage as markdown.
version: 1.0.1
metadata:
  openclaw:
    homepage: https://github.com/Aurora-Ocean-King/wpdtc-skills-one#wpdtc-url-to-markdown
    requires:
      anyBins:
        - bun
---
...
Fetches any URL via `wpdtc-fetch` CLI (Chrome CDP + site-specific adapters) and converts it to clean markdown.
...
4. `${READER}` = `{baseDir}/scripts/wpdtc-fetch`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.wpdtc-skills/wpdtc-url-to-markdown/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/wpdtc-skills/wpdtc-url-to-markdown/EXTEND.md` | XDG |
| 3 | `$HOME/.wpdtc-skills/wpdtc-url-to-markdown/EXTEND.md` | User home |
...
- **Q3 — Save** (header "Save"): "Where to save preferences?"
  - "User (Recommended)" — `~/.wpdtc-skills/` (all projects)
  - "Project" — `.wpdtc-skills/` (this project only)
...
| `--chrome-profile-dir <path>` | Chrome user data directory (default: `WPDTC_CHROME_PROFILE_DIR` env or `./wpdtc-skills/chrome-profile`) |
...
The agent must construct the output file path — `wpdtc-fetch` does not auto-generate paths.
...
|----------|-------------|
| `WPDTC_CHROME_PROFILE_DIR` | Chrome user data directory (can also use `--chrome-profile-dir`) |
