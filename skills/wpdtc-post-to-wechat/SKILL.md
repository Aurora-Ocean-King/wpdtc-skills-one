...
metadata:
  openclaw:
    homepage: https://github.com/Aurora-Ocean-King/wpdtc-skills-one#wpdtc-post-to-wechat
    requires:
      anyBins:
        - bun
        - npx
...
| Path | Scope |
|------|-------|
| `.wpdtc-skills/wpdtc-post-to-wechat/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/wpdtc-skills/wpdtc-post-to-wechat/EXTEND.md` | XDG |
| `$HOME/.wpdtc-skills/wpdtc-post-to-wechat/EXTEND.md` | User home |
...
| Profile dir | Shared profile at `wpdtc-skills/chrome-profile` |
...
| API credentials | Follow guided setup in Step 2, or set in `.wpdtc-skills/.env` |
...
**API selected + missing credentials** → run guided setup per `references/api-setup.md` (writes to `.wpdtc-skills/.env`).
