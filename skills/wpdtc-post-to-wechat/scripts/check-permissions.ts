...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.wpdtc-skills', '.env');
  const userEnv = path.join(os.homedir(), '.wpdtc-skills', '.env');
...
async function main(): Promise<void> {
  console.log('=== wpdtc-post-to-wechat: Permission & Environment Check ===\n');
