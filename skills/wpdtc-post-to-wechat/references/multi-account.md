...
3. `.wpdtc-skills/.env` with the prefixed key `WECHAT_{ALIAS}_APP_ID`
4. `~/.wpdtc-skills/.env` with the prefixed key
...
```bash
# Account: wpdtc
WECHAT_WPDTC_APP_ID=your_wechat_app_id
WECHAT_WPDTC_APP_SECRET=your_wechat_app_secret
...
${BUN_X} {baseDir}/scripts/wechat-article.ts --markdown <file> --theme default --account wpdtc
${BUN_X} {baseDir}/scripts/wechat-browser.ts --markdown <file> --images ./photos/ --account wpdtc
```
