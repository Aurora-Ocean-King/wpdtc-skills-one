...
2. `<cwd>/.wpdtc-skills/.env` with `WECHAT_APP_ID=...`
3. `$HOME/.wpdtc-skills/.env` with `WECHAT_APP_ID=...`
...
Where to save?
A) Project-level: .wpdtc-skills/.env (this project only)
B) User-level: ~/.wpdtc-skills/.env (all projects)
```
