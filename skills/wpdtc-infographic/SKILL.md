...
metadata:
  openclaw:
    homepage: https://github.com/Aurora-Ocean-King/wpdtc-skills-one#wpdtc-infographic
---
...
- If `usage: direct` AND the chosen backend accepts reference images (e.g., `wpdtc-imagine` via `--ref`) → pass the file via the backend's ref parameter
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.wpdtc-skills/wpdtc-infographic/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/wpdtc-skills/wpdtc-infographic/EXTEND.md` | XDG |
| 3 | `$HOME/.wpdtc-skills/wpdtc-infographic/EXTEND.md` | User home |
...
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure wpdtc-infographic preferences" / "重新配置"). The next run re-triggers first-time setup.
