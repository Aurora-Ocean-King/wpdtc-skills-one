...
metadata:
  openclaw:
    homepage: https://github.com/Aurora-Ocean-King/wpdtc-skills-one#wpdtc-slide-deck
    requires:
      anyBins:
        - bun
        - npx
...
At generation time, verify files exist. If `usage: direct` and the backend accepts refs (e.g., `wpdtc-imagine --ref`), pass the file on every slide. Otherwise embed extracted `style`/`palette` traits in the prompt text.
...
| Path | Scope |
|------|-------|
| `.wpdtc-skills/wpdtc-slide-deck/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/wpdtc-skills/wpdtc-slide-deck/EXTEND.md` | XDG |
| `$HOME/.wpdtc-skills/wpdtc-slide-deck/EXTEND.md` | User home |
