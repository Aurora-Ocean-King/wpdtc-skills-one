...
const DEFAULT_PROVIDER_RATE_LIMITS: Record<Provider, ProviderRateLimit> = {
  ...
  evolink: { concurrency: 2, startIntervalMs: 2000 },
};
...
Options:
  ...
  --provider google|openai|openrouter|dashscope|zai|minimax|replicate|jimeng|seedream|azure|evolink  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
  ...
  --ref <files...>          Reference images (Google, OpenAI, Azure, OpenRouter, Replicate supported families, MiniMax, Seedream 4.0/4.5/5.0, DashScope wan2.7-image*, or Evolink with public image URLs)
...
Environment variables:
  ...
  EVOLINK_API_KEY           Evolink API key (get it at https://evolink.ai/dashboard/keys)
  EVOLINK_IMAGE_MODEL       Default Evolink model (gpt-image-2)
  EVOLINK_BASE_URL          Custom Evolink endpoint
  WPDTC_IMAGE_GEN_MAX_WORKERS  Override batch worker cap
  WPDTC_IMAGE_GEN_<PROVIDER>_CONCURRENCY  Override provider concurrency
  WPDTC_IMAGE_GEN_<PROVIDER>_START_INTERVAL_MS  Override provider start gap in ms

Env file load order: CLI args > EXTEND.md > process.env > <cwd>/.wpdtc-skills/.env > ~/.wpdtc-skills/.env`);
}
...
async function loadEnv(): Promise<void> {
  ...
  const homeEnv = await loadEnvFile(path.join(home, ".wpdtc-skills", ".env"));
  const cwdEnv = await loadEnvFile(path.join(cwd, ".wpdtc-skills", ".env"));
...
function getExtendConfigPathPairs(cwd: string, home: string): ExtendConfigPathPair[] {
  return [
    {
      current: path.join(cwd, ".wpdtc-skills", "wpdtc-imagine", "EXTEND.md"),
      legacy: path.join(cwd, ".wpdtc-skills", "wpdtc-image-gen", "EXTEND.md"),
    },
    {
      current: path.join(home, ".wpdtc-skills", "wpdtc-imagine", "EXTEND.md"),
      legacy: path.join(home, ".wpdtc-skills", "wpdtc-image-gen", "EXTEND.md"),
    },
  ];
}
...
export function getConfiguredMaxWorkers(extendConfig: Partial<ExtendConfig>): number {
  const envValue = parsePositiveInt(process.env.WPDTC_IMAGE_GEN_MAX_WORKERS);
...
export function getConfiguredProviderRateLimits(
  ...
  const configured: Record<Provider, ProviderRateLimit> = {
    ...
    evolink: { ...DEFAULT_PROVIDER_RATE_LIMITS.evolink },
  };

  for (const provider of ["replicate", "google", "openai", "openrouter", "dashscope", "zai", "minimax", "jimeng", "seedream", "azure", "evolink"] as Provider[]) {
    const envPrefix = `WPDTC_IMAGE_GEN_${provider.toUpperCase()}`;
...
export function detectProvider(args: CliArgs): Provider {
  if (
    ...
    args.provider !== "evolink"
  ) {
    throw new Error(
      "Reference images require a ref-capable provider. Use --provider google (Gemini multimodal), --provider openai (GPT Image edits), --provider azure (Azure OpenAI), --provider openrouter (OpenRouter multimodal), --provider replicate, --provider dashscope with a wan2.7 image model, --provider seedream for supported Seedream models, --provider minimax for MiniMax subject-reference workflows, or --provider evolink (requires public image URLs)."
    );
  }
  ...
  const hasEvolink = !!process.env.EVOLINK_API_KEY;
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, AZURE_OPENAI_API_KEY+AZURE_OPENAI_BASE_URL, OPENROUTER_API_KEY, DASHSCOPE_API_KEY, ZAI_API_KEY, MINIMAX_API_KEY, REPLICATE_API_TOKEN, JIMENG keys, ARK_API_KEY, or EVOLINK_API_KEY.\n" +
      "Create ~/.wpdtc-skills/.env or <cwd>/.wpdtc-skills/.env with your keys."
  );
}
...
function shouldAllowRemoteReferenceImages(provider: Provider | null): boolean {
  return provider === "dashscope" || provider === "evolink";
}
...
async function loadProviderModule(provider: Provider): Promise<ProviderModule> {
  ...
  if (provider === "evolink") return (await import("./providers/evolink")) as ProviderModule;
...
function getModelForProvider(
  ...
  if (extendConfig.default_model) {
    ...
    if (provider === "evolink" && extendConfig.default_model.evolink) return extendConfig.default_model.evolink;
  }
