...
const scriptPath = path.join(repoRoot, "skills", "wpdtc-imagine", "scripts", "build-batch.ts");
...
async function makeFixture(): Promise<{
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "wpdtc-imagine-build-batch-"));
...
test("build-batch omits default model so wpdtc-imagine can resolve env or EXTEND defaults", async () => {
