---
name: wpdtc-imagine
description: AI image generation with OpenAI GPT Image 2, Azure OpenAI, Google, OpenRouter, DashScope, Z.AI GLM-Image, MiniMax, Jimeng, Seedream, Evolink and Replicate APIs. Supports text-to-image, reference images, aspect ratios, and batch generation from saved prompt files. Sequential by default; use batch parallel generation when the user already has multiple prompts or wants stable multi-image throughput. Use when user asks to generate, create, or draw images.
version: 1.0.1
metadata:
  openclaw:
    homepage: https://github.com/Aurora-Ocean-King/wpdtc-skills-one#wpdtc-imagine
    requires:
      anyBins:
        - bun
        - npx
...
Official API-based image generation. Supports OpenAI GPT Image 2, Azure OpenAI, Google, OpenRouter, DashScope (阿里通义万象), Z.AI GLM-Image, MiniMax, Jimeng (即梦), Seedream (豆包), Evolink (异步 GPT Image 2) and Replicate.
...
| Path | Scope |
|------|-------|
| `.wpdtc-skills/wpdtc-imagine/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/wpdtc-skills/wpdtc-imagine/EXTEND.md` | XDG |
| `$HOME/.wpdtc-skills/wpdtc-imagine/EXTEND.md` | User home |
...
Legacy compatibility: if `.wpdtc-skills/wpdtc-image-gen/EXTEND.md` exists and the new path doesn't, the runtime renames it to `wpdtc-imagine`. If both exist, the runtime leaves them alone and uses the new path.
...
| `--provider google\|openai\|azure\|openrouter\|dashscope\|zai\|minimax\|jimeng\|seedream\|replicate\|evolink` | Force provider (default: auto-detect) |
...
| `--imageSize 1K\|2K\|4K` | Image size for Google/OpenRouter/Evolink (default: from quality) |
...
| `--ref <files...>` | Reference images. Supported by Google multimodal, OpenAI GPT Image edits, Azure OpenAI edits (PNG/JPG only), OpenRouter multimodal models, Replicate supported families, MiniMax subject-reference, Seedream 5.0/4.5/4.0, DashScope `wan2.7-image-pro`/`wan2.7-image`, Evolink (public HTTP/HTTPS image URLs). Not supported by Jimeng, Seedream 3.0, SeedEdit 3.0, or any DashScope model outside the `wan2.7-image*` family |
...
| `<PROVIDER>_IMAGE_MODEL` | Per-provider model override (`OPENAI_IMAGE_MODEL`, `GOOGLE_IMAGE_MODEL`, `DASHSCOPE_IMAGE_MODEL`, `ZAI_IMAGE_MODEL`/`BIGMODEL_IMAGE_MODEL`, `MINIMAX_IMAGE_MODEL`, `OPENROUTER_IMAGE_MODEL`, `REPLICATE_IMAGE_MODEL`, `JIMENG_IMAGE_MODEL`, `SEEDREAM_IMAGE_MODEL`, `EVOLINK_IMAGE_MODEL`) |
...
| `WPDTC_IMAGE_GEN_MAX_WORKERS` | Override batch worker cap |
| `WPDTC_IMAGE_GEN_<PROVIDER>_CONCURRENCY` | Per-provider concurrency (e.g., `WPDTC_IMAGE_GEN_REPLICATE_CONCURRENCY`) |
| `WPDTC_IMAGE_GEN_<PROVIDER>_START_INTERVAL_MS` | Per-provider start-gap |

**Load priority**: CLI args > EXTEND.md > env vars > `<cwd>/.wpdtc-skills/.env` > `~/.wpdtc-skills/.env`
...
| Evolink (GPT Image 2, async polling, public image URLs for refs) | `references/providers/evolink.md` |
...
2. `--provider` specified → use it (if `--ref`, must be google/openai/azure/openrouter/replicate/seedream/minimax/evolink)
3. Only one API key present → use that provider
4. Multiple keys → default priority: Google → OpenAI → Azure → OpenRouter → DashScope → Z.AI → MiniMax → Replicate → Jimeng → Seedream → Evolink
...
For Evolink, `normal` maps to `quality=medium` + `resolution=1K`; `2k` maps to `quality=high` + `resolution=2K`. Default model is `gpt-image-2-beta`. Evolink is **asynchronous**: the API returns a task ID, the provider polls until completion, then downloads the result. Expect longer generation times vs. synchronous providers. See `references/providers/evolink.md` for the full model list (GPT Image 2, Nanobanana, Seedream, Z Image Turbo, etc.).
...
- Evolink: supported ratios `1:1`, `2:3`, `3:2`, `3:4`, `4:3`, `4:5`, `5:4`, `9:16`, `16:9`, `1:2`, `2:1`, `1:3`, `3:1`, `9:21`, `21:9`; also supports explicit pixel sizes. Reference images must be public HTTP/HTTPS URLs
...
| Input is `outline.md` + `prompts/` (e.g. from `wpdtc-article-illustrator`) | Batch — use `scripts/build-batch.ts` to assemble the payload | The outline + prompt files already contain everything needed |
...
| `references/providers/evolink.md` | Evolink GPT Image 2, async polling, URL references |
