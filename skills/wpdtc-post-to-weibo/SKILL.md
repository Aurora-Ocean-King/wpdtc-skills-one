...
metadata:
  openclaw:
    homepage: https://github.com/Aurora-Ocean-King/wpdtc-skills-one#wpdtc-post-to-weibo
    requires:
      anyBins:
        - bun
        - npx
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.wpdtc-skills/wpdtc-post-to-weibo/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/wpdtc-skills/wpdtc-post-to-weibo/EXTEND.md` | XDG |
| 3 | `$HOME/.wpdtc-skills/wpdtc-post-to-weibo/EXTEND.md` | User home |
...
If a script fails with `Chrome debug port not ready` or `Unable to connect`, kill only the CDP Chrome instances (those with `--remote-debugging-port` AND the wpdtc-skills profile), then retry:

```bash
pkill -f "remote-debugging-port.*wpdtc-skills/chrome-profile" 2>/dev/null; sleep 2
```

**CRITICAL**: Never kill all Chrome processes (`pkill -f "Google Chrome"`). Only kill Chrome instances launched by CDP with the wpdtc-skills profile directory. The user may have regular Chrome windows open.
